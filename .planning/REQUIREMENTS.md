...
- [x] **READER-10**: TL;DR strip and the "Anchored to (resolved)" excerpt render the Editor-Agent's markdown as formatted text (not literal markup) via a safe plugin-local renderer, and `<PREFIX>-NNN` references inside the TL;DR are rewritten inline to "ID — title" using the 07-01 SDK resolver (instance-agnostic). **(Implemented — Plan 07-02; live-verified on BEAAA 2026-05-29: `clarity-ref-card-quote` renders h3/p/a/em/strong (no literal `## `); TL;DR strip renders p/strong/code with `BEAAA-828 — CEO opens 3 capacity conversations`; SafeMarkdown has no `dangerouslySetInnerHTML` + href allowlist.)**
