...
- [ ] 01-05-PLAN.md — Safety CLI embedded-postgres + Windows defect cleanup (from Plan 02-01 Task 2 spike findings): pg_dump locator with bundled-binary-first discovery, server/client version pre-check, dbUrl auto-derivation for embedded-postgres mode, new `runbook/operator-gotchas.md` catalog. Wave 4, mixed (Tasks 1-3 autonomous + Task 4 non-autonomous verification on local Windows clone). **Pending** — does NOT gate BEAAA (Hostinger uses hosted Postgres; defect 2/3 only bite the embedded-postgres path used for Windows-side dev work).
