...
- [x] 01-05-PLAN.md — Safety CLI embedded-postgres + Windows defect cleanup (from Plan 02-01 Task 2 spike findings): pg_dump locator with bundled-binary-first discovery, server/client version pre-check, dbUrl auto-derivation for embedded-postgres mode, new `runbook/operator-gotchas.md` catalog. Wave 4, mixed (Tasks 1-3 autonomous + Task 4 verification on local Windows clone). **Done 2026-05-13** — defect-1 fixed in commit `e26f874` (mode-detect schema drift); defect-2 fixed in commits `e26f874+f5c7410+dc79738` (locator + dbUrl derivation + snapshot/cli glue); defect-3 documented + clean error path shipped (inherent pg_dump version-match constraint, not programmatically fixable). Verification: snapshot CLI invoked end-to-end against local embedded-postgres clone, VersionMismatchError fired with runbook-linked message exactly as designed. Full safety CLI test suite 122/122 pass. SUMMARY: pending `01-05-SUMMARY.md` after all Task 4 artifacts commit.
