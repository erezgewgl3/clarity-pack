...
**PHASE 17 SEED — fold in two Reader legibility fixes Eric flagged live on v1.5.1 (2026-06-10):** (1) the goal-ancestry breadcrumb dumps the FULL company-mission goal text on every issue (`deriveAncestry` uses `goal.title` which is the whole mission) AND its links 404 (prefix-less `/goals/<id>`, `/issues/<id>` — Paperclip needs `/<companyPrefix>/…`); (2) the "ANCHORED TO" ref-cards read too code-heavy (lead with `BEAAA-NNN` + `Stuck`/`Standby` chips). Phase 17 was roadmapped around the human-wait signal, so explicitly ADD these to discuss-phase 17 scope. Full diagnosis + fix sketch: memory `reader-breadcrumb-legibility-bug` + MemPalace clarity_pack/decisions.
...
Wired into `finalizeTldr` AFTER `stripMetaProse` and AFTER the min-length gate, BEFORE `upsertTldr`. Cosmetic only — no sentence drops or semantic changes. 22 new tests including the verbatim BEAAA-1000 v1.1.8-output slop sample transforming correctly across all three passes.

**Tarball:** `clarity-pack-1.1.9.tgz` sha256 `b29f9661f3b8123d0c66fc16f842bee8aa6d0c72cdde9d40249c433b807e5058`. Deployed → `status=ready version=1.1.9 id=a763176a`.

---

## 📌 FUTURE TECH DEBT — Option B: structured TL;DR output (deferred)

The operator's instinct (2026-05-30): "the template may be too rigid. We don't know what type of future tasks or issues the company's agents will have to handle." Correct call — deferred.

**The temptation** after v1.1.9's deterministic wins is to escalate to fully structured output: have the agent return JSON like `{ headline_sentence, current_state, blocker, next_action_for_eric }` and render template-style. That would give us 100% control over voice and zero risk of meta-prose drift.

**Why it's deferred:** rigid templates fail on heterogeneous issue types. Not every issue has a "next action" — research summaries are observations; status updates are state reports; post-mortems are retrospective; questions await input. A fixed `{ headline, state, blocker, next_action }` shape would force every issue into the same mould and either fabricate a `next_action_for_eric` or leave the field awkwardly empty.

**Right design when this gets picked up:** a **discriminated union of templates** the agent classifies into, plus optional fields with smart rendering, plus a free-prose escape hatch:

```ts
type TldrShape =
  | { kind: 'decision';  headline; current_state; blocker?; next_action_for_eric }
  | { kind: 'status';    headline; what_changed[]; impact?;  next_check_in? }
  | { kind: 'research';  headline; findings[]; open_questions[]; recommendation? }
  | { kind: 'question';  headline; question;  context?; what_eric_should_consider[] }
  | { kind: 'postmortem'; headline; what_happened; root_cause; what_changed_to_prevent }
  | { kind: 'freeform'; body };  // escape hatch when none of the above fits
```

The agent's prompt becomes: "Classify the issue into one of these shapes; fill the required fields; use `freeform` ONLY when none of the above fits." Renderer maps each kind to a consistent template. Free-prose escape hatch ensures we never lose information just because the shape didn't match.

**Prereq before picking this up:** sample 30-50 real BEAAA issues across the agent corpus, classify by type, see which templates emerge organically. The 5 listed above are a starting guess; the actual taxonomy will probably differ.

**Trigger to revisit:** if v1.1.9's deterministic pipeline still leaves you reading slop in the wild, OR if a new issue type emerges that the prompt rules can't shape (e.g. a TL;DR for a multi-step playbook that needs to render as a checklist).

---

## ✅ v1.1.8 SHIPPED on BEAAA — 2026-05-30 (TRANSLATOR role + 5 voice rules)

The Editor-Agent's prompt was reframed: not "summarize" but **TRANSLATE** (agent-internal vocabulary → busy-founder English). Five hard voice rules: direct "you" address; active verbs/present tense; concrete over nominal; human dates; translate every agent term. New BAD/GOOD example pair grounded in BEAAA-1000 with explicit per-failure-mode annotations. Effect was real but partial — agent moved ~30% toward the target voice and stalled on training-distribution defaults (ISO dates, restated parens, generic jargon). v1.1.9 picks up those tails deterministically.

Tarball `d1fc6e50eb90434973e4959c7257431d3edc564c4d2d6332a869f79b81ed9ead` (commit 7c336a3).

---

## ✅ v1.1.7 SHIPPED on BEAAA — 2026-05-30 (deterministic meta-prose strip; LLM trust dropped)
...
- Markdown structure (bullets / headings / paragraphs) survives.
- A line mixing meta + substance keeps the substantive sentence.
- Short stub bodies (test fixtures) that don't match any pattern are NOT rejected by the gate.

Suite 2244/2247 — only the known pre-existing situation-artifacts failure remains.

**Tarball:** `clarity-pack-1.1.7.tgz` sha256 `8225b1ebd960345ae67460b7bac137687216019c3bbfaaa7f49bdc779e50bfe0`. Deployed → `status=ready version=1.1.7 id=a763176a-2f4d-4986-b190-b5151e42cc00`.

**Cache invalidation path — operator-gated:** BEAAA-1000's cached TL;DR (the bad one from 2026-05-30 07:53) won't change until either (a) its body or a comment changes (the `content_hash` includes both — cache miss → recompile with v1.1.6 prompt + v1.1.7 strip), or (b) the cache row is manually deleted via SQL (requires DB env, which the autoclassifier blocked this session). Recommended: pick a low-stakes test issue (BEAAA-585 "Actual Test", or any throwaway) → add a one-line comment → open the Reader → observe the recompiled TL;DR. If the agent's output trips a meta pattern, the gate fires (TL;DR stays in `compiling` state and retries on next view).

---

## ✅ v1.1.6 SHIPPED on BEAAA — 2026-05-30 (Editor-Agent TL;DR content rule)
...
**Effect is heartbeat-dependent.** The new prompt only kicks in on the next recompile of each issue (when the issue body or comments change). BEAAA-1000's CACHED TL;DR will keep showing the old meta-prose until the agent recompiles. To force-recompile sooner, edit the issue body or add a comment.

**Tarball:** `clarity-pack-1.1.6.tgz` sha256 `5402217961bea3ff51b93d3a521089bbf1a2faa29717d98825b8ccfa03d845e9`. Deployed → `status=ready version=1.1.6 id=a763176a-2f4d-4986-b190-b5151e42cc00`.

---

## ✅ v1.1.5 SHIPPED & LIVE-VERIFIED on BEAAA-1000 — 2026-05-30 (hide operation issues at resolve-refs)
...
- `src/ui/primitives/ref-chip.tsx` — when `card.hiddenAsRef`, render a plain `<span class="clarity-ref-chip-hidden">{card.id}</span>`. No chip frame, no status badge, no anchor, no hover-peek. Branch sits AFTER the loading degrade and BEFORE the anchor render.

**Tests:** 5 new worker behavioural tests (mixed batch, prefix-not-exact match across operation kinds, null-originKind treated as normal) + 6 new ref-chip source-grep tests (ordering, no anchor/peek/chip-class in the hidden branch, safety: no `href=`). Existing reference-resolver + ref-chip-title + ref-chip-peek all green.

**Live-verification on BEAAA-1000:** `hiddenCount: 1, hiddenIds: ['BEAAA-1168']`, `chipCount: 2` (BEAAA-1086 + BEAAA-1103, the real-task sign-off chips, still render with full titles + DONE badges). The TL;DR PROSE still has "TL;DR stored as the compile-result document on BEAAA-1168 and the operation issue is marked done." — that's the v1.1.6 prompt fix's job (above), not the chip's.

**Tarball:** `clarity-pack-1.1.5.tgz` sha256 `8809c2068df866fdefd08deb374bd6db76123f059a21b66d1eebbea63457dda1`. Deployed → `status=ready version=1.1.5`.

---

## ✅ v1.1.4 SHIPPED & LIVE-VERIFIED on BEAAA-1000 — 2026-05-30 (chip visual identity)
...
- `statusBg`: `oklch(0.62 0.18 25)` (solid red), `statusColor`: dark contrast, `borderRadius`: `999px`. Pill shape.
- Status text: `"blocked"`.

The chip now reads as: red-bar + monospace id + sans/serif title + solid red pill. Unmistakably a labelled link, not a code span.
...
## ✅ v1.1.3 SHIPPED & LIVE-VERIFIED across 5 BEAAA issues — 2026-05-30 (leading-id code-span split)

**Closes the "still reads like a mess" complaint on BEAAA-1000.** The pervasive agent pattern — code spans like `` `BEAAA-1086 — UW operational pre-read of BEAAA-1000 Scope-β engagement plan` `` — now chips BOTH the leading id AND any embedded refs in the gloss. The user's screenshot showed boxed-monospace strings everywhere with no chips; v1.1.3 deploys an explicit reversal of v1.1.1's conservative "mixed code stays code" call.

**Root cause:** v1.1.1's parser upgrade only fired for code spans whose ENTIRE content was a `PREFIX-NNN` token. The agent's actual writing style — `<id> <separator> <gloss>` — was mixed content, intentionally left untouched. That left the operator with a wall of boxed-monospace text everywhere a ref was cited, which is exactly the "rabbit hole" the chip pipeline was meant to close.

**Fix (commit 0274fb1):** a code span whose content STARTS with a valid `PREFIX-NNN` token followed by a separator (whitespace / em-dash `—` / en-dash `–` / colon) now chips the leading id AND recursively parses the trailing gloss so embedded refs in the gloss also chip. Hyphen (`-`) and dot (`.`) are NOT separators so derived tokens (`BEAAA-933-extension`, `BEAAA-933.json`) and legit code (`npm test`, `v1.1.2`, `in_review`, status enums, `b5c5da95`) stay untouched. Internally: `Match.span: InlineSpan` widened to `Match.spans: InlineSpan[]` so the code-span split can emit `[ref, ...glossSpans]` in one match step.

**Tests:** 12 new behavioural tests (BEAAA-1000's exact agent patterns, separator set, derived tokens stay code, off-prefix narrowing in prefix-narrowed mode, broad fallback, back-compat without refOpts, chained-ref gloss, ref-span carries only a validated id with XSS-safe gloss discard). 1 v1.1.1 test updated to reflect the explicit reversal (`MIXED content with leading PREFIX-NNN + em-dash IS broken apart`). Suite 2216/2220 — only the known pre-existing `situation.artifacts` failure remains; chat-messages U7 watchdog and disabled sketch-regression are flaky-under-load.

**Tarball:** `clarity-pack-1.1.3.tgz` sha256 `6267d98144753f6fbbc878c241d67736e054ef08201c8ed086ee86fc80d74117`. Deployed → `status=ready version=1.1.3 id=a763176a-2f4d-4986-b190-b5151e42cc00`.

**Live verification across 5 distinct BEAAA issues, all 0 leftover patterns:**

| Issue | Domain | TL;DR chips | Page-wide chips | Leftover `<id> — gloss` boxes |
|---|---|---|---|---|
| BEAAA-1000 | CTO Scope-β (the user's screenshot) | 3 (BEAAA-1168 / BEAAA-1086 / BEAAA-1103) | 14 | **0** |
| BEAAA-1047 | UW op-seat blocker tracker (prior complaint) | 5 (BEAAA-1047 / BEAAA-933 ×4) | 56 | **0** |
| BEAAA-924 | HoUW countersign | 6 (BEAAA-918 / BEAAA-617 / BEAAA-187 / BEAAA-829) | 35 | **0** |
| BEAAA-918 | Actuary pre-status pressure test (mid-recompile) | n/a (compiling) | 0 (compile in flight) | **0** |
| BEAAA-585 | Actual Test (empty fixture) | n/a (compiling) | 0 | **0** |

Chip titles include full agent-relevant context (e.g. `BEAAA-1086 — UW operational pre-read of BEAAA-1000 Scope-β engagement plan (for 2026-06-03 CTO ↔ HoUW review) — DONE`). Screenshot: `v1.1.3-BEAAA-1000-chips.png`.

---

## ✅ v1.1.2 SHIPPED & LIVE-VERIFIED on BEAAA — 2026-05-30 (bold-headline recursion)

**The user's "still reads like a mess" complaint on BEAAA-1047 (Reader TL;DR) is closed.** The bold headline now resolves `**BEAAA-1047 is blocked on countersigning [BEAAA-933](/BEAAA/issues/BEAAA-933)**` to two titled chips (BEAAA-1047 + BEAAA-933, each with full title + status badge) inside the `<strong>`. Previously the markdown link rendered as literal text inside the bold.

**Root cause:** SafeMarkdown's `strong` / `em` / `link` spans carried a flat `text` / `label` STRING that the .tsx renderer mounted as a raw text node. Nested markdown (refs, links, code) was invisible to the recursion. Live verification on v1.1.1 caught this — list items below the headline rendered chips correctly, but the bold headline still showed the raw `[BEAAA-933](/BEAAA/issues/BEAAA-933)` syntax.

**Fix (commit e07dc2f):** `InlineSpan.strong / .em / .link` now carry `spans: InlineSpan[]`. `firstInlineMatch` recursively calls `parseInline` on the inner content; the renderer maps each span through `renderInline`. `code` and `ref` stay leaves (code is verbatim; ref carries only a validated id). The link → ref upgrade contract is unchanged — a canonical `[PREFIX-NNN](/<prefix>/issues/PREFIX-NNN)` still becomes a single ref span (not a link wrapping a ref). Non-canonical links and the labels of canonical links with extra content now contain nested ref / code / em / bold spans.

**Tests:** 2 existing tests updated to the new `spans` shape; 6 new behavioural tests: link-canonical-inside-bold → ref, em-with-ref-inside, non-canonical link with bold + ref nested, triply-nested `**outer _BEAAA-9_ end**`, code stays verbatim leaf, XSS holds with `javascript:` href inside bold. Suite 2206/2209 (only the known pre-existing `situation.artifacts` failure).
...
- `anchorsInStrong: 2` — both chips are clickable anchors. Screenshot `v1.1.2-bold-headline-chips-resolved.png`.

**OUT-OF-SCOPE DISCOVERY (not us, not fixed):** the 43+ console `/api/issues/PAGE-3` / `STAGE-1` / `STAGE-2` 404s on BEAAA-1047 come from the HOST's `paperclip-markdown-issue-ref` autolinker matching plain prose tokens ("page-3", "Stage 1") against its own broad ref pattern. EVERY clarity-pack chip on the page has a valid `BEAAA-NNN` id (53/53 clean). Fix belongs upstream in paperclipai. Filed in MemPalace `clarity_pack/runbook` for future operator.

---

## ✅ v1.1.1 SHIPPED & LIVE-VERIFIED on BEAAA — 2026-05-30 (Reader chip pipeline)

**Two complementary fixes for the BEAAA-1047 "rabbit hole" complaint** — bare ids without titles where the Reader's ref-chip pipeline should have resolved them.

1. **safe-markdown parser** (commit 68b6a0d) — a code span that is JUST a `PREFIX-NNN` token AND a markdown link whose label is a bare `PREFIX-NNN` with matching `/<prefix>/issues/<sameId>` href are now upgraded to ref spans (so the chip resolves the title). Mixed-content code (`` `BEAAA-933 — gloss` ``), custom-label links, cross-instance links, and links with extra path/query stay their original kind — conservative upgrade. XSS allowlist (sanitizeHref) is unchanged.
2. **Editor-Agent TL;DR prompt** (commit dd66abc) — three contract lines: cite issue ids as plain prose (no backticks, no markdown link wrapping); never restate the cited issue's title/status next to its id (the chip already shows them); expand every internal abbreviation or jargon term on first use (concrete examples for HoUW, AC4). The parser fix shows visible win on the existing cached TL;DR immediately; the prompt fix takes effect on the next agent heartbeat recompile.
3. **Truncation reserve dynamic** (commit 580624c) — the 1.1.1 prompt-contract addition pushed the prompt scaffolding past the prior fixed 1200-char reserve, so the truncated body overshot by ~56 tokens on three truncation tests. Replaced with a dynamic `scaffolding-length + TLDR_TRUNCATION_MARKER.length` reserve so any future prompt edit auto-adjusts.

**Tarball:** `clarity-pack-1.1.1.tgz` sha256 `816081e28c69357de7bce041b8f9c0eed6eafd4f8e48ce0462042b63fd51463d`. Deployed → `status=ready version=1.1.1`. Live-verify caught the bold-headline recursion bug (closed by v1.1.2 above).

---

## ✅ v1.1.0 SHIPPED & LIVE-VERIFIED on BEAAA — 2026-05-29

**Three post-1.0.0 fixes from the 2026-05-29 diagnostic session, shipped as v1.1.0 via `/gsd:quick` (NEXT-SESSION-BRIEF.md).** All built, gated, deployed, and live-verified on BEAAA.

1. **Pin-no-brick UI** (`bf08f90`) — `onPin`/`onPromote` in `message-thread.tsx` made optimistic with an 8s safety timeout. Root cause: under box load the `chat.pin` ACK was measured at 45s+ while the write still persisted, so the `disabled={busy}` button stayed stuck on `cursor:not-allowed`. onPin now flips the optimistic marker + toast immediately, fires `pin()` without awaiting inside the busy window, and resets busy via BOTH `.finally()` and an 8s `setTimeout`. Tests 12/12.
2. **requestWakeup de-block** (`9c4370c`) — the two awaited sites (`chat-send.ts` send-ACK + `agent-task-delivery.ts startAgentTask` step 3, the bulletin/TL;DR storm source) are now fire-and-forget. `requestWakeup` is unreliable on paperclipai@2026.525.0 (30s timeout / scope errors); native wake is the real delivery mechanism. Never-resolving-stub tests prove both handlers resolve promptly. Tests 35/35.
3. **Launchers** (`1f112d8`) — `ui.launchers` (sidebar zone) surfaces Situation Room / Daily Bulletin / Employee Chat as left-nav entries (previously reachable only by direct URL). Verified against host source: `placementZone:'sidebar'` is the only persistent-left-nav zone, requires the `ui.sidebar.register` capability, and `navigate` `action.target` is the BARE routePath (host prepends `/<companyPrefix>/`). Manifest test ML1-ML5.

**Version bump 1.0.0 → 1.1.0** (package.json + manifest.ts literal + release note); version-guard tests widened to the 1.x family. Test-contract follow-up `3b6ca57`.

**Gates ALL GREEN:** tsc clean; check-css-scope 164 scoped; worker(2.5MB)+UI(707.7 kB)+manifest builds OK; check-ui-bundle-size 724,709 / 733,184 ceiling + 0 SheetJS; full suite 2185 tests, only the 1 known pre-existing `situation.artifacts` fixture failure (the `fail 2` count includes its file wrapper); `grep -c paperclipInvocation dist/worker.js` = 5.
...
**One open item:** `git push origin master` was blocked by the auto-mode guardrail (direct default-branch push). The 4 commits (`bf08f90`, `9c4370c`, `1f112d8`, `3b6ca57`) are on LOCAL master only — the deploy used the local tarball, so BEAAA is current, but the remote is behind until Eric pushes (or grants push permission). Pre-existing host 501 on the optional chat *stream* bridge persists (polling fallback works) — not ours, not a blocker.

---

## 🏁 v1.0.0 COMPLETE for INTERNAL use — 2026-05-29

**All 7 phases shipped + live-verified on BEAAA.** The four surfaces (Reader, Situation Room, Daily Bulletin, Employee Chat) + the Editor-Agent run live at v1.0.0 (id=a763176a) on tarball `f34c5eee…` / 715,097 bytes, status=ready.

**Operator decision 2026-05-29 — internal-only, NO npm publish.** clarity-pack is used solely on operator-controlled Paperclip instances and distributed via the local-tarball `paperclipai plugin install <package-dir>` path (DEPLOY-RUNBOOK Path A/B). **DIST-01 (publish to public npm) is a deliberate WON'T-DO**, and Plan 05-10's operator-gated publish step is dropped. The plugin packaging (`paperclipPlugin` field → dist/manifest.js + dist/worker.js + dist/ui/) is complete; only the *public registry push* is skipped.
...
4. **Test gate:** 1 pre-existing `situation.artifacts` unit-test failure (documented, long-standing); the visual-regression test fails on headless-browser Google-Fonts network (gate behind `SKIP_VISUAL=1`).

---

## ✅ PHASE 7 COMPLETE & VERIFIED LIVE on BEAAA — 2026-05-29 (Plan 07-05 gloss read-back fix shipped + re-drilled)

**The last Phase 7 nub is closed.** Plan 07-05's bulletin gloss DISPLAY bug (BUG 2 — diagnosed in the prior session) is FIXED, re-deployed, and re-drilled to a 4/4 PASS. BULL-10 → Implemented; all 5 Phase 7 chunks (07-01/02/03/04/05) are CLOSED & VERIFIED LIVE.

**The bug (now fixed):** `driveBulletinGlossStep` spawned a NEW operation issue every Bulletin view and did ONE immediate poll that always returned `pending` — the Editor-Agent compiled the brand-new op ~40s later (async) AND marked it `done` per `RESULT_DELIVERY_INSTRUCTION`. So `startAgentTask`'s idempotency search (which reuses only NON-terminal ops) skipped the now-terminal op and spawned a fresh one whose immediate poll was again `pending`. The agent's stored `compile-result` was never read back → permanent "Gloss pending…" + a recompile per view.

**The fix (commit `dad114b`, option (b) — lowest blast radius, touches ONLY `src/worker/bulletin/bulletin-gloss.ts`, NOT the shared `agent-task-delivery.ts`):** on a cache-miss, BEFORE `startAgentTask`, the gloss step lists the existing op(s) for this cycle (`originId=bulletin-gloss-<cycle>`, INCLUDING terminal/done ones) and reads back the stored `compile-result` via the shared `pollAgentTaskResult` document path; a ready body → `finalizeTldr` into `tldr_cache` + apply; only spawn a new op if none is found. **TDD:** a two-view RED→GREEN test (`test/worker/bulletin/bulletin-gloss.test.mjs` — "compile lands on a LATER view") — view 1 creates an op + pending; the agent stores the result + marks done; view 2 reads back the SAME op, finalizes, applies the gloss, with NO duplicate op. **No version bump (1.0.0), no migration, no new deps.**

**Gates ALL GREEN:** tsc clean; check-css-scope 164 scoped; worker(2.5MB)+UI(723,313B)+manifest builds OK; check-ui-bundle-size 723,313 / 724,992 (UNCHANGED — the fix is worker-only) + 0 SheetJS; full suite 2154 / 2149 pass / 1 fail (documented pre-existing situation.artifacts) / 4 skip with the network-only visual test skipped (the visual test's lone failure is a headless-browser Google-Fonts `networkidle` timeout — not a regression); `grep -c paperclipInvocation dist/worker.js` = 5.
...
4. **Heading/scope-fence** — count-aware heading; masthead + 4 department summaries + Standing Numbers (Open 151 / Completed·7d 256 / Blocked 24 / Spend $0 / Budget 0.0%) render unchanged.

Screenshots: `07-05-readback-view1-gloss-displays.png`, `07-05-readback-view2-gloss-persists-cachehit.png`.

**NEXT (operator-gated):** Plan 05-10 — rc → 1.0.0 npm publish + Phase 5 / milestone close. `npm publish` is a public, irreversible release — confirm with Eric before publishing. The live BEAAA install already carries the v1.0.0 label.

---

## Plan 07-05 CODE-COMPLETE 2026-05-29 — deploy + live BEAAA drill ORCHESTRATOR-PENDING (LAST Phase 7 chunk)
...
- **LineageFooter render (Task 2, D-I5-02/03/04):** `src/ui/surfaces/bulletin/lineage-footer.tsx` reframes the heading to `Work in motion — N thread(s)`, renders `thread.gloss` (or a quiet `Gloss pending…` note), and adds TWO per-thread affordances — Open issue (`/<prefix>/issues/<identifier>`) + Open chat with owner (reused ROOM-09 `buildChatDeepLink({route:'employee-only'})` carrier; owner-gated). React text nodes only; no `dangerouslySetInnerHTML`; entityId/ownerAgentId never rendered as text. Scoped CSS under `[data-clarity-surface="bulletin"]`. **BULL-10** added to REQUIREMENTS.md (Pending).
- **Plumbing:** `resolveEditorAgentId` EXPORTED from editor.ts (one-word back-compat); `'bulletin-gloss'` added to the `OperationKind` union + a JSON-object readback gate in `agent-task-delivery.ts`; `LineageThread` gains optional `gloss`/`identifier`/`ownerAgentId`.

**Constraints honoured:** NO version bump (1.0.0 in package.json + src/manifest.ts:337 + dist/manifest.js); NO migration (tldr_cache reuse — latest is 0014, untouched); **no new runtime dep**.

**Commits (master):** `93fb085` (Task 1 pure filter + view-driven gloss step, RED→GREEN) → `3628a84` (Task 2 handler filter+enrich+gloss + LineageFooter heading/gloss/2-affordances + scoped CSS + BULL-10, RED→GREEN) → docs commit.

**Deviation — AUTHORIZED bundle-ceiling recalibration 704→708 kB.** The LineageFooter delta (gloss element + 2 affordances + count-aware heading + the now-required `useHostNavigation`/`useHostLocation`/`buildChatDeepLink`/`extractCompanyPrefixFromPathname` wiring) pushed the built UI bundle 719,502 → **723,313 B** (+3,811 B; +2,417 B over the prior 704 kB ceiling, overflowing the ~1.4 kB 07-03 headroom). The filter + gloss step are worker-side (zero UI cost) + the type fields are type-only. Confirmed **zero SheetJS sentinels**; bumped `UI_BUNDLE_BYTES_CEILING` 704→708 kB (724,992 B, ~1.7 kB headroom) with a dated justification comment, per the 05-04/05-11/07-02/07-03/07-04 precedent. The locked feature surface (D-I5-02/03/04) was NOT crippled to fit.

**Gates ALL GREEN:** tsc clean; check-css-scope 164 scoped; worker(2.5MB)+UI(723,313B)+manifest builds OK; check-ui-bundle-size 723,313 / 724,992 (recalibrated) + 0 SheetJS; full suite **2156 total / 2153 pass / 1 fail (pre-existing situation.artifacts) / 2 skip**; `grep -c paperclipInvocation dist/worker.js` = 5; `lineage-filter.ts` pure (no `ctx.`) + exports filterLineageThreads/isRoutineThread + no `BEAAA`; `bulletin-gloss.ts` reuses getTldrByScope/finalizeTldr/tldrContentHash + surface:'bulletin' + no `resume` call + no `BEAAA`; `bulletin-by-cycle.ts` calls filterLineageThreads + driveBulletinGlossStep (the dead compile-bulletin JOB untouched); `lineage-footer.tsx` no `dangerouslySetInnerHTML`.

**Tarball:** `clarity-pack-1.0.0.tgz` sha256 `54e517e6b4fa386c7c7d1ba37a0eaeea5237ec769e1f4c6ef83d82f8f5f87f63`, 714,536 bytes, 18 files (0 src/test/png leaks).

**NEXT (ORCHESTRATOR, PRE-AUTHORIZED — do NOT pause):** deploy the tarball to BEAAA via DEPLOY-RUNBOOK Path A (2 SSH connections; bookended-by-snapshots satisfied by DO daily backup + rehearsed Phase 1 restore) + run the live BEAAA Bulletin Playwright drill from a LOCAL window. PRIMARY proofs: (1) the lineage threads NO LONGER include the routine/scheduled outputs (Daily Founder digest / Daily CEO status report ×2 / Nightly Auditor Report) NOR exact dups; (2) each surviving thread shows a one-line gloss (Editor-Agent running) or a calm "Gloss pending…" — NO raw UUID in any gloss text; (3) both per-thread affordances exist + the Open-chat lands on the right owner. Fill the 4 TBD verdicts in `07-05-SUMMARY.md`; **BULL-10 flips to Implemented** after the drill confirms steps 1–3. **Phase 7 is COMPLETE after this drill;** then Plan 05-10 (rc→1.0.0 npm publish + Phase 5/milestone close — operator-gated).

---

## Plan 07-03 CODE-COMPLETE 2026-05-29 — deploy + live BEAAA drill ORCHESTRATOR-PENDING
...
- **Banner/panel UI (Task 2):** NEW `org-blocked-backlog-banner.tsx` — top-of-room "N blocked · M need you" banner (aria-expanded, auto-expand when M>0) → expandable panel; each row = title + human action + owner NAME (`ownerName ?? 'Unassigned'`, never the UUID) + age (`blocked {formatAge}`) + two affordances (Open issue `/<prefix>/issues/<id>` + "Open chat with <owner>" via the reused ROOM-09 `buildChatDeepLink({route:'employee-only'})` carrier; disabled when owner is null/__unowned__) + overflow footer. Mounted ABOVE the room header in `index.tsx` (agent grid unchanged). React text only; no `dangerouslySetInnerHTML`. CSS `.clarity-blocked-*` scoped under `[data-clarity-surface='situation-room']`. **ROOM-12** added to REQUIREMENTS.md (Pending).

**Constraints honoured:** NO version bump (1.0.0 in package.json + src/manifest.ts:337 + dist/manifest.js); NO migration (the backlog is COMPUTED, not stored — latest is 0014, untouched); **no new runtime dep**.

**Commits (master):** `f437191` (Task 1 pickTopChains export + builder, RED→GREEN) → `e892efc` (Task 2 handler wire-in + banner UI + ROOM-12, RED→GREEN) → docs commit.

**Deviation — AUTHORIZED bundle-ceiling recalibration 696→704 kB.** As the plan-checker anticipated (it flagged the plan's stale 694 kB ceiling text; the actual constant was already 696 kB / 712,704 B with ~1.3 kB headroom), the banner overflowed it: the built UI bundle went 711,429 → **719,502 B** (+8,073 B). The banner + its mount are the ONLY new UI-bundle code (the builder is worker-side; pickTopChains moved into a shared module the UI doesn't import). Confirmed zero SheetJS sentinels; bumped `UI_BUNDLE_BYTES_CEILING` 696→704 kB (720,896 B, ~1.4 kB headroom) with a dated justification comment, per the 05-04/05-11/07-02/07-04 empirical-recalibration precedent. The locked banner surface (D-I4-01..04) was NOT crippled to fit.

**Gates ALL GREEN:** tsc clean; check-css-scope 164 scoped; worker(2.4MB)+UI(719,502B)+manifest builds OK; check-ui-bundle-size 719,502 / 720,896 (recalibrated) + 0 SheetJS; full suite **2109 total / 2106 pass / 1 fail (pre-existing situation.artifacts) / 2 skip**; `grep -c paperclipInvocation dist/worker.js` = 5; `org-blocked-backlog.ts` imports flattenBlockerChain+pickTopChains + NO BEAAA literal; the dead JOB does NOT compute org_blocked_backlog (HANDLER does).

**Tarball:** `clarity-pack-1.0.0.tgz` sha256 `b3dc5d37c7f1b165a00955880a2699ecc0117814f8a8d2f549dadb75b5154f41`, 711,589 bytes, 18 files (0 src/test/png leaks).

**NEXT (ORCHESTRATOR, PRE-AUTHORIZED — do NOT pause):** deploy the tarball to BEAAA via DEPLOY-RUNBOOK Path A (2 SSH connections; bookended-by-snapshots satisfied by DO daily backup + rehearsed Phase 1 restore) + run the live BEAAA Situation Room Playwright drill from a LOCAL window. PRIMARY proofs: (1) the org-blocked banner shows a NON-ZERO blocked count (BEAAA has ~24), NOT "No blockers" everywhere; (2) ≥1 row renders title + a single human action + an owner display NAME (or "Unassigned", NEVER a raw UUID); (3) the two per-row affordances exist. Fill the TBD verdicts in `07-03-SUMMARY.md`; **ROOM-12 flips to Implemented** after the drill confirms steps 1–3.

---

## Plan 07-04 CODE-COMPLETE 2026-05-29 — deploy + live BEAAA drill ORCHESTRATOR-PENDING
...
- **Worker rewrite REMOVED (D-I31-04):** the 07-02 `tldr-ref-titles.ts` module + its test DELETED; the issue-reader import + the buildTitleMap/inlineRefTitles try/catch block DELETED; PRIM-01 reverted 6→3 gets. The TL;DR body reaches the UI raw and the chip is the sole title source — no double-render. `grep tldr-ref-titles|inlineRefTitles|buildTitleMap` over src/+test/ = ZERO.

**Constraints honoured:** NO version bump (1.0.0 in package.json + src/manifest.ts:337 + dist/manifest.js); no migration; **no new runtime dep**.

**Commits (master):** `2cbe858` (Task 1 RefChip title) → `e073e63` (Task 2 SafeMarkdown ref-awareness) → `249aa15` (Task 3 render-everywhere rewrite) → `d237ddf` (Task 4 worker-rewrite removal) → docs commit.

**Chat ripple (flag for orchestrator):** `ProseWithRefChips` is also consumed by `message-thread.tsx` (lines 720, 974); chat message bodies now render formatted markdown + titled chips. Intended unified rendering; `{ body }` prop shape unchanged so neither call site changed. A chat message with literal `**` now renders bold — acceptable; the orchestrator should confirm the chat surface is not visually regressed at the live drill.

**Deviation — AUTHORIZED bundle-ceiling recalibration 694→696 kB.** The ref-aware delta pushed the built UI bundle 709,383 → 711,429 B (+2,046 B), overflowing the ~1,273 B 07-02 headroom by 773 B. Confirmed zero SheetJS sentinels; bumped `UI_BUNDLE_BYTES_CEILING` 694→696 kB (712,704 B, ~1.2 kB headroom) with a justification comment, per the 05-04/05-11/07-02 empirical-recalibration precedent.

**Gates ALL GREEN:** tsc clean; check-css-scope 145 scoped; worker(2.4MB)+UI(711,429B)+manifest builds OK; check-ui-bundle-size 711,429 / 712,704 (recalibrated) + 0 SheetJS; full suite **2070 total / 2067 pass / 1 fail (pre-existing situation.artifacts) / 2 skip**; `grep -c paperclipInvocation dist/worker.js` = 5; grep tldr-ref-titles|inlineRefTitles|buildTitleMap = ZERO.

**Tarball:** `clarity-pack-1.0.0.tgz` sha256 `b8f061e99dcfe7fb57c649ccd67005e22e36893a3b8892c2cc4bb39bb3b0ba7b`, 709,191 bytes, 18 files (0 src/test/png leaks).

**NEXT (ORCHESTRATOR, PRE-AUTHORIZED — do NOT pause):** deploy the tarball to BEAAA via DEPLOY-RUNBOOK Path A (2 SSH connections; bookended-by-snapshots satisfied by DO daily backup + rehearsed Phase 1 restore) + run the live BEAAA-828 Playwright drill from a LOCAL window. PRIMARY proofs: (1) `clarity-reader-prose` renders FORMATTED markdown (h3/strong/ul/em/code), NO literal `## `/`**`; (2) in-prose refs are clickable `ID — title` chips. Also: TL;DR chips titled (FLAG if agent paused), NO `ID — title — title` double-render, chat surface not regressed. Fill the TBD verdicts in `07-04-SUMMARY.md`; READER-03/READER-10 reaffirmed (already Implemented).

---

## Plan 07-02 CODE-COMPLETE 2026-05-29 — deploy + live BEAAA drill ORCHESTRATOR-PENDING
...
**Constraints honoured:** NO version bump (1.0.0 in package.json + src/manifest.ts:337 + dist/manifest.js); no migration; **no new runtime dep** (the renderer is hand-rolled, plugin-local).

**Commits (master):** `6efeb05` (Task 1 SafeMarkdown + XSS guard) → `7bced21` (Task 2 refs→title post-processor + wire-in) → `673772f` (Task 3 both render targets + tightened prompt + READER-10) → docs commit.

**Deviation — AUTHORIZED bundle-ceiling recalibration 688→694 kB.** SafeMarkdown (the only UI-bundle addition) pushed the built UI bundle 703,238 → 709,383 B (+6,145 B), overflowing the ~1,274 B 07-01 headroom. Confirmed zero SheetJS sentinels; bumped `UI_BUNDLE_BYTES_CEILING` 688→694 kB (710,656 B, ~1.3 kB headroom) with a justification comment, per the 05-04/05-11 empirical-recalibration precedent.

**Gates ALL GREEN:** tsc clean; check-css-scope 140 scoped; worker(2.4MB)+UI(709,383B)+manifest builds OK; check-ui-bundle-size 709,383 / 710,656 (recalibrated) + 0 SheetJS; full suite **2062 total / 2059 pass / 1 fail (pre-existing situation.artifacts) / 2 skip**; `grep -c paperclipInvocation dist/worker.js` = 5.

**Tarball:** `clarity-pack-1.0.0.tgz` sha256 `46897f7a283bc8a5837de6594c5280e013bc6e92c1df500d5c0b6fcbc54c56ac`, 709,225 bytes, 18 files (0 src/test/png leaks).

**NEXT (ORCHESTRATOR, PRE-AUTHORIZED — do NOT pause):** deploy the tarball to BEAAA via DEPLOY-RUNBOOK Path A (bookended-by-snapshots satisfied by DO daily backup + rehearsed Phase 1 restore; minimize SSH connections per the fail2ban lesson) + run the live BEAAA Playwright drill from a LOCAL window. The drill's PRIMARY proof is step 1 — the Anchored-to excerpt on BEAAA-828 now renders FORMATTED markdown (verifiable regardless of Editor-Agent state). Step 2 (TL;DR strip markdown + refs-as-titles) is verifiable only if the Editor-Agent is RUNNING; a paused agent showing "Compiling…" is the explicit-resume-only lock, NOT a regression (record the agent state). READER-10 flips to Implemented after the drill confirms step 1. SUMMARY: `07-02-SUMMARY.md`. Items 4-5 + Plan 05-10 (npm publish) get their own gates.
...
Last activity: 2026-06-19 — Quick task 260619-r4v: chat create-task topic-linkage redesign + agent-feedback loop closure (3 pieces, v1.8.10, --validate 7/7 + load-bearing anti-storm guards); LOCAL ONLY on branch quick/chat-loop-closure-260619, not deployed, live drill pending Eric's go-ahead
...
| Phase 11 P03 | ~18 minutes | 2 tasks | 6 files |
...
| Phase 21 P04 | 25 min | 2 tasks | 3 files |
...
| 260619-eyw | Fix recurring "wrong agent runs the TL;DR" — gate `startAgentTask` creation-time `requestWakeup` on `!reused` so Reader ~5s reuse-polls stop each firing a wake (~12/min for one stuck compile) that auto-engaged the durable wake-governor kill-switch (>6/min, never auto-clears) → editor wakes all suppressed → op falls to recovery sweep → escalates to CTO/CEO (editor never runs, 0 tokens). One distinct compile = exactly one wake now; 6/min ceiling caps DISTINCT issues, not poll cadence. Provenance write stays unconditional on reuse. v1.8.8→1.8.9 (load-bearing: `wake-kill-switch-repo.isEngaged` is version-scoped, so v1.8.9 ignores CM's stale engaged row — auto-recovers on deploy, no DB surgery). TDD RED→GREEN; suite 2960 pass / typecheck 0. wake-governor + kill-switch design untouched (removes storm source, not the safety cap). | 2026-06-19 | e6812d8 | [260619-eyw-gate-creation-time-requestwakeup-on-reus](./quick/260619-eyw-gate-creation-time-requestwakeup-on-reus/) |
| 260619-r4v | Chat create-task topic-linkage redesign + agent-feedback loop closure (3 pieces): topic-required dialog (Standalone removed, default open topic, inline +New topic, atomic `newTopicTitle`, cold path removed — fixes COU-56 invisibility); company-wide assignee-scoped Active-tasks-owned rail (live-assignee grouping so a reassigned task follows its owner, M=100, showing-N-of-M); in-thread live task-update cards via READ-TIME reflection (new `chat.topicTaskUpdates` handler — latest agent comment + live status + blocked→named-action). Load-bearing anti-storm guards (ZERO issues.list/write/requestWakeup/event-subs, populated+empty) passing. v1.8.10. --validate: plan-check PASS, verify 7/7 (3 live-drill items pending Eric). LOCAL ONLY — NOT deployed. branch `quick/chat-loop-closure-260619` | 2026-06-19 | 46d705d | [260619-r4v-chat-create-task-topic-linkage-agent-fee](./quick/260619-r4v-chat-create-task-topic-linkage-agent-fee/) |
