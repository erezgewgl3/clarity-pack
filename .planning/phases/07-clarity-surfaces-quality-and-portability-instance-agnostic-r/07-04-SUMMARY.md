...
The executor does NOT deploy in the build task. The orchestrator window (which holds the authenticated localhost:3100 tunnel + BEAAA SSH + Playwright MCP) runs this AFTER the build. Deploy is PRE-AUTHORIZED (memory `autonomous-deploy-authorization`; bookended-by-snapshots satisfied by the DO daily backup + the rehearsed Phase 1 restore — no manual pre-deploy snapshot). Deploy `clarity-pack-1.0.0.tgz` sha256 `b8f061e99dcfe7fb57c649ccd67005e22e36893a3b8892c2cc4bb39bb3b0ba7b` via DEPLOY-RUNBOOK Path A as a **2-connection burst** (upload `rm`-then-`cat`-over-stdin one connection — `fs.protected_regular` blocks even root from truncating the beai-agent-owned /tmp tarball, so `rm` first is mandatory; install here-string one connection — avoids the fail2ban trip the 07-01 5-connection burst hit). Radix tabs need a real `browser_click`; read DOM via `browser_evaluate`.

Open the BEAAA-828 Reader tab and assert ALL of:

1. **Main prose body renders formatted markdown — PRIMARY PROOF — verdict TBD.** `browser_evaluate` the `clarity-reader-prose` DOM: it contains real formatted elements (h3/h4 / strong / ul+li / em / code) and NO text node with a literal `## ` or `**`. (This is the operator's "rest of the reader looks half rendered" complaint — it must now be formatted.)
2. **In-prose refs are clickable titled chips — PRIMARY PROOF — verdict TBD.** Inside `clarity-reader-prose`, the ref chips render `ID — title` (a chip whose text matches `/BEAAA-704 — .+/`, NOT `BEAAA-704 · done`, NOT plain `BEAAA-704`), clickable anchors to `/<prefix>/issues/<id>`.
3. **TL;DR refs are ALSO clickable titled chips — PROOF — verdict TBD.** In `clarity-tldr-body`, any `PREFIX-NNN` renders as the SAME titled clickable chip. **FLAG:** if the Editor-Agent is PAUSED the TL;DR shows "Compiling…/paused" and this step is not falsifiable (the explicit-resume-only lock, NOT a regression) — record the agent state; rely on steps 1+2 + the excerpt as the live proof; the unit tests cover the chip render.
4. **NO double-render — PROOF — verdict TBD.** NO chip/text shows `ID — title — title` anywhere (worker rewrite gone; chip is the sole title source). `browser_evaluate` a regex over the rendered text for `/-\s—\s.+\s—\s/` near a ref → absent.
5. **Excerpt + scope-fence sanity — verdict TBD.** The Anchored-to `clarity-ref-card-quote` still renders formatted markdown (07-02 proof holds) and its in-quote refs are now titled chips too; the Reader does not show "Clarity Pack: failed to render".
6. **Chat surface not regressed — verdict TBD (07-04 ripple).** Open a chat topic; a message body renders formatted markdown + titled chips with no console key warnings and no "failed to render". (See the "Chat ripple" flag above — literal `**` now renders bold; acceptable.)
7. **XSS — proof-of-record (unit).** No live hostile fixture expected on BEAAA; the Task-2 unit XSS guards (no `javascript:` href; `<script>` inert; no `dangerouslySetInnerHTML`) are the proof of record.
