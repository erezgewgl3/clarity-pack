...
| `Open chat with [Role]` opens chat but doesn't pre-select the agent | The deployed plugin build predates Plan 06.1-12 v2 (parser fix for employee-only payloads). Re-install `clarity-pack-1.0.0-rc.8` (SHA256 `23181e4a…`) — confirmed by the URL hash decoding to `{"employee":"<uuid>"}` and the chat header showing the agent name. |
