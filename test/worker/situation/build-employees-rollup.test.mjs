...
test('rollup — 11-03: split identity — awaitedPartyLabel has NO raw UUID; targetAgentUuid/targetIssueUuid carry the UUID (NO_UUID_LEAK)', async () => {
  ...
  // targetAgentUuid = the agent UUID (mutation-only), affordance 'nudge' (Phase 21
  // 21-CONTEXT D-1; was 'assign' under Plan 12-01 D-05). The leaf node is a real
  // UUID so targetIssueUuid is UUID-shaped. The rendered awaitedPartyLabel must be
  // scrubbed of every raw UUID.
  ...
  assert.equal(row.blockerChain.actionAffordance, 'nudge', "AWAITING_AGENT_STUCK affordance is 'nudge' (Phase 21 21-CONTEXT D-1)");
});
